...
const GestHospedesFacade_1 = require("../../core/GestHospedesFacade");
describe('GestHospedesFacade - Testes Unitários', () => {
    let facade;
    let mockUtilizadorDAO;
    let mockAnimalDAO;
    beforeEach(() => {
        facade = new GestHospedesFacade_1.GestHospedesFacade();
        mockUtilizadorDAO = facade.utilizadorDAO;
        mockAnimalDAO = facade.animalDAO;
    ...
    describe('Método: criarContaTutor', () => {
        ...
        it('deve lançar erro se o email não possuir um formato válido (ex: sem @)', async () => {
            await expect(facade.criarContaTutor('João', 'joaoemail.com', 'pwd_hash_123', '123456789', '912345678')).rejects.toThrow('Formato de email inválido.');
        ...
        it('deve lançar erro se já existir uma conta registada com o mesmo email', async () => {
            ...
            await expect(facade.criarContaTutor('Maria', 'maria@email.com', 'pwd_hash_123', '987654321', '912345678')).rejects.toThrow('Já existe uma conta com este Email!');
            expect(mockUtilizadorDAO.createTutor).not.toHaveBeenCalled();
        });
    });
    describe('Método: registarAnimal', () => {
        ...
        it('deve gerar um microchip provisório caso o animal não possua microchip', async () => {
            ...
            mockAnimalDAO.createAnimal.mockImplementation((dados) => Promise.resolve({ idAnimal: 'animal-2', ...dados }));
        ...
        it('deve garantir que o plano vacinal submetido é guardado com isValido = false e estado = Valido', async () => {
            ...
            expect(mockAnimalDAO.createAnimal).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
                isValido: false,
                estado: 'Valido'
            }));
        });
