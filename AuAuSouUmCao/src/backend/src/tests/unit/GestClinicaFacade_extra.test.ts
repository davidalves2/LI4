import { GestClinicaFacade } from '../../core/GestClinicaFacade';
...
describe('GestClinicaFacade - Testes Adicionais (Cobertura Extra)', () => {
  let facade: GestClinicaFacade;
  let mockStockDAO: any;
  let mockPrescricaoDAO: any;

  beforeEach(() => {
    facade = new GestClinicaFacade();

    mockStockDAO      = (facade as any).stockDAO;
    mockPrescricaoDAO = (facade as any).prescricaoDAO;
  ...
  describe('Método: ativarQuarentena', () => {
    ...
    it('deve ativar a quarentena com motivo válido', async () => {
      ...
      expect((resultado as any).emQuarentena).toBe(true);
  ...
  describe('Métodos de listagem e delegação', () => {
    ...
    it('desativarQuarentena deve delegar ao DAO com o ID correto', async () => {
      ...
      expect((resultado as any).emQuarentena).toBe(false);
    ...
    it('finalizarTratamento deve delegar ao DAO com o idLinha correto', async () => {
      ...
      expect((resultado as any).estado).toBe('Finalizado');
